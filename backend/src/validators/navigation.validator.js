...
const updateNavigationRules = [
  body("items").isArray().withMessage("Navigation items must be an array."),

  body("items.*.id")
    .notEmpty()
    .withMessage("Each navigation item must have an id."),

  body("items.*.label")
    .trim()
    .notEmpty()
    .withMessage("Each navigation item must have a label."),
  ...
  body("items.*.url").optional({ values: "null" }),
