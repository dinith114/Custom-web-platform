// BuilderToolbar — top bar with Back, undo/redo, device toggle, Save/Preview/Publish
"use client";
...
import { IconArrowLeft, IconEye } from "@/components/shared/Icons";
  ...
  return (
    <div className="flex h-12 items-center justify-between border-b border-gray-200 bg-white px-4">
      ...
      <div className="flex items-center gap-2">
        {isDirty && <span className="text-xs text-amber-500 font-medium mr-1">● Unsaved</span>}
        {saveStatus === "saved" && <span className="text-xs text-green-500 font-medium mr-1">✓ Saved</span>}

        <button
          onClick={handleSave}
          disabled={saveStatus === "saving"}
          className="rounded-lg px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-100 transition-colors disabled:opacity-50 flex items-center gap-1.5"
        >
        ...
        <button
          ...
          className="rounded-lg bg-brand-600 px-4 py-1.5 text-sm font-medium text-white hover:bg-brand-700 transition-colors disabled:opacity-50 flex items-center gap-1.5"
        >
