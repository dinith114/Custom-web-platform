// Settings page — site settings form with mock data
"use client";
    ...
    <div>
      ...
      <div className="mb-6 flex gap-1 rounded-lg bg-gray-100 p-1 max-w-md">
        {(["general", "seo", "danger"] as const).map((tab) => (
          <button
            ...
            className={`flex-1 rounded-md px-3 py-2 text-sm font-medium transition-colors ${activeTab === tab ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
          >
            {tab === "general" ? "⚙️ General" : tab === "seo" ? "🔍 SEO" : "⚠️ Danger Zone"}
          </button>
        ))}
      </div>
      ...
      {activeTab === "general" && (
        <div className="mx-auto max-w-2xl space-y-6">
          <div className="rounded-xl border border-gray-200 bg-white p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input type="text" value={customDomain} onChange={(e) => setCustomDomain(e.target.value)} placeholder="www.yourdomain.com" className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" />
                <p className="mt-1 text-xs text-gray-400">Point your domain's CNAME to builder.example.com</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Favicon URL</label>
                <input type="text" value={favicon} onChange={(e) => setFavicon(e.target.value)} placeholder="https://..." className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" />
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="flex gap-2 items-center">
                  <input type="color" defaultValue="#6366f1" className="h-9 w-12 cursor-pointer rounded border border-gray-200" />
                  <span className="text-xs text-gray-400 font-mono">#6366f1</span>
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Font Family</label>
                <select defaultValue="Inter" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
                  <option>Inter</option>
          ...
          <div className="flex justify-end">
            <button onClick={handleSave} className="rounded-lg bg-brand-600 px-6 py-2.5 text-sm font-medium text-white hover:bg-brand-700 transition-colors flex items-center gap-2">
      ...
      {activeTab === "seo" && (
        <div className="mx-auto max-w-2xl space-y-6">
          <div className="rounded-xl border border-gray-200 bg-white p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={seoTitle} onChange={(e) => setSeoTitle(e.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" />
                <p className="mt-1 text-xs text-gray-400">{seoTitle.length}/60 characters</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Meta Description</label>
                <textarea value={seoDesc} onChange={(e) => setSeoDesc(e.target.value)} rows={3} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" />
              ...
              <div>
                ...
                <input type="text" value={ogImage} onChange={(e) => setOgImage(e.target.value)} placeholder="https://..." className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" />
              </div>
            </div>

            {/* Preview */}
            <div className="mt-6 rounded-lg border border-gray-200 bg-gray-50 p-4">
              <p className="text-xs font-medium text-gray-400 mb-2 uppercase tracking-wider">Google Preview</p>
              <div>
          ...
          <div className="flex justify-end">
            <button onClick={handleSave} className="rounded-lg bg-brand-600 px-6 py-2.5 text-sm font-medium text-white hover:bg-brand-700 transition-colors">{saved ? "✓ Saved!" : "💾 Save SEO Settings"}</button>
          </div>
        </div>
      )}
      ...
      {activeTab === "danger" && (
        <div className="mx-auto max-w-2xl">
          <div className="rounded-xl border border-red-200 bg-red-50/50 p-6">
            <h3 className="text-sm font-semibold text-red-700 mb-4">⚠️ Danger Zone</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between rounded-lg border border-red-200 bg-white p-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Take your site offline. Visitors will see a 404 page.</p>
                </div>
              ...
              <div className="flex items-center justify-between rounded-lg border border-red-200 bg-white p-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Transfer this site to another user account.</p>
                </div>
              ...
              <div className="flex items-center justify-between rounded-lg border border-red-300 bg-white p-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500">This action cannot be undone. All pages, media, and data will be lost.</p>
                </div>
