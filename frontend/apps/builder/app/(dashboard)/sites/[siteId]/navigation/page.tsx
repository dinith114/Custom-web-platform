// Navigation page — consistent design with SVG icons
"use client";
...
import { IconPlus, IconClose, IconEye } from "@/components/shared/Icons";
...
export default function NavigationPage() {
  ...
  return (
    <div>
      <PageHeader title="Navigation Menu" subtitle="Configure your site's navigation structure." />
      <div className="mx-auto max-w-2xl rounded-xl border border-gray-200 bg-white p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-gray-900">Menu Items ({items.length})</h3>
          <button
        ...
        <div className="space-y-2">
          {items.map((item, idx) => (
            <div key={item.id} className="flex items-center gap-3 rounded-lg border border-gray-200 bg-white px-4 py-3 hover:border-gray-300 transition-colors">
              <span className="cursor-grab text-gray-300 hover:text-gray-500 text-sm">⠿</span>
              ...
              <button
                ...
                className={`rounded-lg p-1.5 transition-colors ${item.isVisible ? "text-green-600 hover:bg-green-50" : "text-gray-400 hover:bg-gray-50"}`}
              >
          ...
          <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input type="text" value={newLabel} onChange={(e) => setNewLabel(e.target.value)} placeholder="About Us" autoFocus className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">URL</label>
                <input type="text" value={newUrl} onChange={(e) => setNewUrl(e.target.value)} placeholder="/about" className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" />
              </div>
              <div className="flex justify-end gap-2 pt-1">
                <button onClick={() => setIsAddOpen(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">Cancel</button>
                <button onClick={addItem} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700 transition-colors">Add Item</button>
              </div>
            </div>
          </div>
        </div>
